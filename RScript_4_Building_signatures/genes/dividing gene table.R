...
signature_genes <- readxl::read_excel("signature_genes.xlsx")
...
for (phenotype in phenotype_categories) {
  for (genotype in genotype_categories) {
    # Filter the original dataframe (signature_genes) according to Phenotype and Genotype
    ...
    if (nrow(filtered_df) > 0) {
      ...
      df_name <- paste("genes_signature", phenotype, genotype, sep = "_")
      
      # Build the full path of the Excel file
      file_path <- file.path("C:/Users/Emamnuell/Desktop/panoptosis/genes/genes_24_optosis/codigos/4°_signature/genes", paste0(df_name, ".xlsx"))
      
      # Export the filtered dataframe to an Excel file using writexl
      writexl::write_xlsx(filtered_df, file_path)
