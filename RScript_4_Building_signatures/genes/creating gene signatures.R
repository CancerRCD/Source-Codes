...
Tabela_Cancer_Genes_Pathways <- import("final_cox_results_genes_com_vias.xlsx")
...
# Function to format the gene signature according to the requirements
...
aggregate_genes <- function(data) {
  ...
  # Combine both aggregated data frames
...
# Create a 'signature_position' column that enumerates the entries for each cancer type
aggregated_df <- aggregated_df %>%
