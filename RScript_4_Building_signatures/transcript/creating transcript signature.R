...
Tabela_Cancer_Genes_Pathways <- import("final_cox_results_transcript_com_vias.xlsx")
...
# Function to aggregate transcripts by cancer type, optosis, phenotypic variable, and genotypic variable, separating by correlation
aggregate_genes <- function(data) {
  ...
  aggregate_and_format <- function(df) {
    df %>%
      ...
      summarise(Transcript = format_gene_signature(Transcript), .groups = 'drop')
  }
...
# Add a column that counts the number of transcripts in each signature
aggregated_df <- aggregated_df %>%
  mutate(number_of_genes = sapply(strsplit(Transcript, " \\+ "), length))
...
export(aggregated_df, "signature_transcript.tsv")
