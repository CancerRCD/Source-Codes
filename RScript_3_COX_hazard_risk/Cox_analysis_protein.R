...
# Dataframe
genes <- import("big_table_contacetado_FxG_protein.xlsx")
...
perform_cox_analysis <- function(genes, measure) {
  ...
  for (gene in unique_genes) {
    ...
    # Create a temporary data frame with current results
    temp_df <- data.frame(
      Gene = gene,
      p.value = p_value,
      Type = Type,
...
export(genes_merged, "final_cox_results_protein.xlsx", row.names = FALSE)
