...
# Dataframe
genes <- import("big_table_contacetado_FxG_genes.xlsx")

# Function to perform Cox analysis for a specific metric
perform_cox_analysis <- function(genes, measure) {
  results <- data.frame()  # Create an empty data frame to store the results
  ...
  for (gene in unique_genes) {
    ...
    # Extract the values of p.value, Type, and cancer
    ...
    # Add the temporary data frame to the main results
    results <- rbind(results, temp_df)
  }
  
  # Rename the columns in results to avoid conflicts when merging
...
# Apply the function for each metric and store the results in the genes data frame
...
for (measure in measures) {
  ...
  end_times[[measure]] <- Sys.time()  # Store the completion time
}

# Save the final result in a CSV file
export(genes_merged, "final_cox_results_genes.xlsx", row.names = FALSE)
