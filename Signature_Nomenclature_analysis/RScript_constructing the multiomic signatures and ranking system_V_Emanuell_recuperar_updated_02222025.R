## Rerun and amended for validation on 05/03/2025 to 07/03/2025 
...
#resultado_com_protein_old <- resultado # only for control

#### remove all rows which value are "protein" under "gene_var"
#resultado <- resultado[resultado$gene_var != "protein", ]

# #Get the  working directory  for the desired protein data fies
# setwd("D:/Pré-artigo 5-optosis model/Dataframes all metrics/df_survival_protein")
# 
# # List all the TSV files in the working directory
# tsv_files_protein <- list.files(pattern = "\\.tsv$")
# 
# # Read the first TSV file with the column names
# df1_protein <- read.delim(tsv_files_protein[1])
# df2_protein <- read.delim(tsv_files_protein[2])
# df3_protein <- read.delim(tsv_files_protein[3])
# df4_protein <- read.delim(tsv_files_protein[4])
# 
# # Order each dataframe alphabetically by the "Gene", "tumor" and "gene_var" columns
# df1_protein <- df1_protein[order(df1_protein$Gene, df1_protein$tumor, df1_protein$gene_var), ]
# df2_protein <- df2_protein[order(df2_protein$Gene, df2_protein$tumor, df2_protein$gene_var), ]
# df3_protein <- df3_protein[order(df3_protein$Gene, df3_protein$tumor, df3_protein$gene_var), ]
# df4_protein <- df4_protein[order(df4_protein$Gene, df4_protein$tumor, df4_protein$gene_var), ]
# 
# # Ensure the first three columns are the same across all dataframes
# 
# # Compare the first three columns of df1_protein with df2_protein
# identical_1_2_protein <- all(df1_protein[, 1:3] == df2_protein[, 1:3])
# 
# # Compare the first three columns of df1_protein with df3_protein
# identical_1_3_protein <- all(df1_protein[, 1:3] == df3_protein[, 1:3])
# 
# # Compare the first three columns of df1_protein with df4_protein
# identical_1_4_protein <- all(df1_protein[, 1:3] == df4_protein[, 1:3])
# 
# # Check if all comparisons are true
# if(identical_1_2_protein && identical_1_3_protein && identical_1_4_protein) {
#   print("The first three columns are identical across all four dataframes.")
# } else {
#   print("The first three columns are NOT identical across all four dataframes.")
# }
# 
# # Detailed comparison (optional, in case of differences)
# if(!identical_1_2_protein) {
#   print("Differences found between df1_protein and df2_protein in the first three columns.")
# }
# if(!identical_1_3_protein) {
#   print("Differences found between df1_protein and df3_protein in the first three columns.")
# }
# if(!identical_1_4_protein) {
#   print("Differences found between df1_protein and df4_protein in the first three columns.")
# }
# 
# # List of dataframes
# dfs_protein <- list(df1_protein, df2_protein, df3_protein, df4_protein)
# 
# # Apply the filtering logic to each dataframe in the list
# filtered_dfs_NS_1_protein <- lapply(dfs_protein, function(df) {
#   df %>%
#     filter(p_val > 0.05, worst_prognosis_group != "NS")
# })
# 
# # Assign the filtered dataframes to individual variables
# filtered_df1_protein_NS_1 <- filtered_dfs_NS_1_protein[[1]]
# filtered_df2_protein_NS_1 <- filtered_dfs_NS_1_protein[[2]]
# filtered_df3_protein_NS_1 <- filtered_dfs_NS_1_protein[[3]]
# filtered_df4_protein_NS_1 <- filtered_dfs_NS_1_protein[[4]]
# 
# # Testing for possible inconsistencies - Apply the filtering logic to each dataframe in the list
# filtered_dfs_NS_2_protein <- lapply(dfs_protein, function(df) {
#   df %>%
#     filter(p_val < 0.05, worst_prognosis_group == "NS")
# })
# 
# # Assign the filtered dataframes to individual variables
# filtered_df1_protein_NS_2 <- filtered_dfs_NS_2_protein[[1]]
# filtered_df2_protein_NS_2 <- filtered_dfs_NS_2_protein[[2]]
# filtered_df3_protein_NS_2 <- filtered_dfs_NS_2_protein[[3]]
# filtered_df4_protein_NS_2 <- filtered_dfs_NS_2_protein[[4]]
# 
# DFI_protein <- df1_protein
# DSS_protein <- df2_protein
# OS_protein <- df3_protein
# PFI_protein <- df4_protein
# 
# DFI_protein <- DFI_protein %>% rename(status_DFI = status, p_val_DFI = p_val, log_rank_DFI = log_rank, 
#                                       worst_prognosis_group_DFI = worst_prognosis_group)
# 
# DSS_protein <- DSS_protein %>% rename(status_DSS = status, p_val_DSS = p_val, log_rank_DSS = log_rank, 
#                                       worst_prognosis_group_DSS = worst_prognosis_group)
# 
# OS_protein <- OS_protein %>% rename(status_OS = status, p_val_OS = p_val, log_rank_OS = log_rank, 
#                                     worst_prognosis_group_OS = worst_prognosis_group)
# 
# PFI_protein <- PFI_protein %>% rename(status_PFI = status, p_val_PFI = p_val, log_rank_PFI = log_rank, 
#                                       worst_prognosis_group_PFI = worst_prognosis_group)
# 
# # Unir as colunas de DSS_protein na tabela OS_protein
# resultado_protein <- OS_protein %>%
#   bind_cols(
#     DSS_protein %>%
#       select(status_DSS, p_val_DSS, log_rank_DSS, worst_prognosis_group_DSS)
#   )
# 
# # Unir as colunas de DFI_protein na tabela resultado_protein
# resultado_protein <- resultado_protein %>%
#   bind_cols(
#     DFI_protein %>%
#       select(status_DFI, p_val_DFI, log_rank_DFI, worst_prognosis_group_DFI)
#   )
# 
# # Unir as colunas de PFI_protein na tabela resultado_protein
# resultado_protein <- resultado_protein %>%
#   bind_cols(
#     PFI_protein %>%
#       select(status_PFI, p_val_PFI, log_rank_PFI, worst_prognosis_group_PFI)
...
# print(colnames(resultado))
# print(colnames(resultado_protein))

# # Check if the column names are identical and in the same order
# if (identical(colnames(resultado), colnames(resultado_protein))) {
#   print("The column names and their order are identical.")
# } else {
#   print("The column names or their order are different.")
#   
#   # Optionally, print the differences in column names
#   print(setdiff(colnames(resultado), colnames(resultado_protein)))
#   print(setdiff(colnames(resultado_protein), colnames(resultado)))
# }

# # Check if the column names are identical and in the same order
# if (identical(colnames(resultado), colnames(resultado_protein))) {
#   # Perform rbind if the column names and order are identical
#   resultado_combined <- rbind(resultado, resultado_protein)
#   print("The dataframes have been successfully combined using rbind.")
# } else {
#   print("The column names or their order are different. rbind() cannot be performed.")
#   
#   # Optionally, print the differences in column names
#   print("Columns in 'resultado' but not in 'resultado_protein':")
#   print(setdiff(colnames(resultado), colnames(resultado_protein)))
#   
#   print("Columns in 'resultado_protein' but not in 'resultado':")
#   print(setdiff(colnames(resultado_protein), colnames(resultado)))
# }
# 
# resultado <- resultado_combined # purged from old protein data
...
# # Create the dataframe 'unicos_protein_tumor_genvar' with unique instances
# unicos_protein_tumor_genvar <- resultado_protein %>%
#   select(Gene, tumor, gene_var) %>%  # Select the columns Gene, tumor, and gene_var
#   distinct() %>%  # Keep only the unique combinations
#   arrange(Gene, tumor, gene_var)  # Sort the values alphabetically by Gene, tumor, and gene_var
# 
# # Identify and create a dataframe 'non_unique_protein_tumor_genvar' with the non-unique rows
# non_unique_protein_tumor_genvar <- resultado_protein %>%
#   select(Gene, tumor, gene_var) %>%  # Select the columns Gene, tumor, and gene_var
#   group_by(Gene, tumor, gene_var) %>%  # Group by the selected columns
#   filter(n() > 1) %>%  # Filter out the groups that have more than 1 instance (non-unique)
#   arrange(Gene, tumor, gene_var)  # Sort the values alphabetically by Gene, tumor, and gene_var
...
# df80 <- resultado_com_protein_old
# sum(df80$gene_var == "protein") # 97 rows
...
# # Importar os data frames
# df1_1 <- import("drafts/df_combinado_protein.xlsx")
...
# 
# # Remove parentheses only for simple solo values in the "Signature" column 
# df1_1$Signature <- df1_1$Signature %>%
#   gsub("^\\(([a-zA-Z0-9_-]+)\\)$", "\\1", .)
# 
# # Check if the column names and order are identical between df1 and df1_1
# columns_identical <- identical(names(df1), names(df1_1))

# # Print the result
# if (columns_identical) {
#   print("The column names and their order are identical between df1 and df1_1.")
# } else {
#   print("The column names or their order are NOT identical between df1 and df1_1.")
# }

# # Combine df1 and df1_1 using rbind
# df_combinado_corrigido <- rbind(df1, df1_1)
# 
# # Check the dimensions of the combined dataframe
# print(dim(df_combinado_corrigido))
# 
# rio::export(df_combinado_corrigido, "drafts/df_combinado_corrigido.tsv")

# df1 <- import("drafts/df_combinado_corrigido.tsv")
...
# 
# df20 <- df20 %>%
#   rename(SMC_Rank = HRC_SMC_rank)
# 
# # Duplicate the "HRC_SMC" column and rename the duplicate to "HRC"
# df20 <- df20 %>%
#   mutate(HRC = HRC_SMC)
# 
# df20 <- df20 %>%
#   rename(SMC = HRC_SMC)
# 
# # Reorder the columns in df20
# df20 <- df20 %>%
#   select(HRC, SMC, series, HRC_Rank, SMC_Rank)

# # Count the number of NA values in the HRC_Rank column
# num_NA <- sum(is.na(df20$HRC_Rank))
# 
# num_NA <- sum(is.na(df20$SMC_Rank))
...
# # Define the list of values to match for Table example signature per genotype
# nomenclature_values <- c("KIRC-169.2.1.P.2.72.82.1.1.2",
#                          "KIRP-107.3.2.N.1.45.45.1.1.2",
#                          "CESC-69.6.3.N.2.45.45.1.1.1",
#                          "BRCA-693.5.3.N.2.93.85.1.1.1",
#                          "UCEC-898.7.3.N.3.21.85.1.1.1",
#                          "BRCA-1496.1.3.P.3.72.72.1.1.2",
#                          "LGG-3206.4.3.N.1.45.35.1.1.3")
# 
# # Subset df70 based on the exact matches in the 'Nomenclature' column
# df73 <- df70[df70$Nomenclature %in% nomenclature_values, ]
# 
# signatures_for_figures <- df73
# rio::export(signatures_for_figures, "signatures_for_figures_7.tsv")
...
####################
####################
# Genes in top signatures with evidence as immunotherapy targets (via chatGPT) 
# URL: https://chatgpt.com/share/66e76273-091c-8012-be5c-67bef35113b2

#### Initialize an empty dataframe to store the results
# immunotherapy_genes_df <- data.frame(
#   Gene = character(),
#   Immunotherapy_Relevance = character(),
#   Details = character(),
#   stringsAsFactors = FALSE
# )
# 
# # Function to add batch results to the dataframe
# add_batch_results <- function(batch_data) {
#   immunotherapy_genes_df <<- rbind(immunotherapy_genes_df, batch_data)
# }
# 
# # Batch 1 results (add your previous batch results here if necessary)
# batch_1 <- data.frame(
#   Gene = c("ACTC1", "ACTL6A", "ADAMTS12", "ADAMTS8", "APBB1", 
#            "APOBEC3G", "ATOH8", "BCL2L14", "CACNA2D2", "CASC15"),
#   Immunotherapy_Relevance = c("No", "No", "Indirect", "Indirect", "No", 
#                               "Indirect", "No", "Indirect", "No", "No"),
#   Details = c("No direct relevance in immunotherapy", 
#               "Involved in chromatin remodeling but no direct immunotherapy evidence", 
#               "Extracellular matrix remodeling, may affect immune infiltration", 
#               "Extracellular matrix remodeling, may affect immune infiltration", 
#               "Involved in amyloid processing, no direct immunotherapy evidence", 
#               "Role in tumor mutational burden, affecting immune responses", 
#               "No direct relevance in immunotherapy", 
#               "Regulates apoptosis, potential relevance in immune evasion", 
#               "Calcium channel regulation, no direct relevance", 
#               "Long non-coding RNA involved in cancer, no direct immunotherapy evidence"),
#   stringsAsFactors = FALSE
# )
# 
# # Add batch 1 to the dataframe
# add_batch_results(batch_1)
# 
# # Batch 2 results
# batch_2 <- data.frame(
#   Gene = c("CCL19", "CCNA2", "CD2", "CD244", "CD8A", "CDC25C", 
#            "CDH5", "COL1A1", "COL4A1", "COL4A2"),
#   Immunotherapy_Relevance = c("Indirect", "No", "Indirect", "Direct", "Direct", 
#                               "No", "No", "No", "No", "No"),
#   Details = c("Involved in recruiting immune cells, potential use in modulating immune infiltration",
#               "Cell cycle regulation, no direct immunotherapy relevance",
#               "Enhances T-cell responses, but not a primary immunotherapy target",
#               "Targeted for enhancing NK cell responses in immunotherapy",
#               "Key molecule in cytotoxic T-cell activity, direct role in immunotherapy",
#               "Involved in cell cycle progression, no direct immunotherapy relevance",
#               "Cell adhesion in endothelial cells, no direct immunotherapy relevance",
#               "ECM component, may affect immune infiltration but no direct relevance",
#               "ECM component, no direct immunotherapy relevance",
#               "Involved in basement membrane integrity, no direct immunotherapy relevance"),
#   stringsAsFactors = FALSE
# )
# 
# # Add batch 2 to the dataframe
# add_batch_results(batch_2)
# 
# # Batch 3 results
# batch_3 <- data.frame(
#   Gene = c("CXADR", "CXCL10", "CYP11A1", "DEPDC1B", "DLGAP5", "DLL4", 
#            "FAM171B", "FOXC2", "GBP1", "GMPS"),
#   Immunotherapy_Relevance = c("No", "Direct", "No", "No", "No", "Indirect", 
#                               "No", "Indirect", "Indirect", "No"),
#   Details = c("Receptor for viruses, no direct role in cancer immunotherapy",
#               "Chemokine involved in recruiting T cells, studied in combination with immunotherapy",
#               "Involved in steroidogenesis, no direct immunotherapy relevance",
#               "Involved in cell cycle progression, no direct immunotherapy relevance",
#               "Mitotic spindle protein, no direct immunotherapy relevance",
#               "Key component of Notch signaling, potential use in tumor angiogenesis targeting",
#               "Limited information, no direct immunotherapy relevance",
#               "Involved in EMT and immune evasion, but not a direct immunotherapy target",
#               "Part of interferon pathway, potential role in enhancing antitumor immunity",
#               "Involved in purine biosynthesis, no direct immunotherapy relevance"),
#   stringsAsFactors = FALSE
# )
# 
# # Add batch 3 to the dataframe
# add_batch_results(batch_3)
# 
# # Batch 4 results
# batch_4 <- data.frame(
#   Gene = c("GPR4", "GZMB", "HJURP", "IKZF3", "ITGA3", "ITGB3", 
#            "KIF14", "KIF4A", "KLHDC7B", "KLRK1"),
#   Immunotherapy_Relevance = c("No", "Direct", "No", "Indirect", "No", "Indirect", 
#                               "No", "No", "No", "Direct"),
#   Details = c("Involved in vascular biology, no direct immunotherapy relevance",
#               "Key effector in immune-mediated tumor destruction, relevant in checkpoint therapies",
#               "Involved in DNA repair, no direct role in cancer immunotherapy",
#               "Involved in lymphoid cell regulation, potential relevance in immunomodulatory therapies",
#               "Involved in cell adhesion, no direct immunotherapy relevance",
#               "Plays a role in angiogenesis, potential to influence immune recruitment",
#               "Involved in mitotic spindle formation, no direct immunotherapy relevance",
#               "Involved in chromosome segregation, no direct immunotherapy relevance",
#               "Limited information, no direct immunotherapy relevance",
#               "Crucial receptor for NK and T-cell immune surveillance, promising immunotherapy target"),
#   stringsAsFactors = FALSE
# )
# 
# # Add batch 4 to the dataframe
# add_batch_results(batch_4)
# 
# # Batch 5 results
# batch_5 <- data.frame(
#   Gene = c("LAMA1", "LAMC1", "LTA", "MATN3", "MINPP1", "MIR10A", 
#            "MPRIP", "MRC2", "MSRB3", "NCAPH"),
#   Immunotherapy_Relevance = c("No", "No", "Indirect", "No", "No", "No", 
#                               "No", "No", "No", "No"),
#   Details = c("Involved in ECM structure, no direct immunotherapy relevance",
#               "Involved in ECM structure, no direct immunotherapy relevance",
#               "Cytokine involved in immune responses, potential relevance in combination therapies",
#               "Involved in cartilage formation, no direct immunotherapy relevance",
#               "Involved in phosphoinositide signaling, no direct immunotherapy relevance",
#               "MicroRNA involved in gene regulation, no direct immunotherapy relevance",
#               "Involved in cytoskeletal dynamics, no direct immunotherapy relevance",
#               "Involved in collagen degradation, no direct immunotherapy relevance",
#               "Involved in oxidative stress response, no direct immunotherapy relevance",
#               "Involved in chromosome condensation, no direct immunotherapy relevance"),
#   stringsAsFactors = FALSE
# )
# 
# # Add the fifth batch to the dataframe
# add_batch_results(batch_5)
# 
# # Batch 6 results
# batch_6 <- data.frame(
#   Gene = c("NEK2", "NFIX", "NNMT", "NPHP1", "NPR2", "NR1H3", 
#            "NT5E", "NUDT6", "NUSAP1", "OTX1"),
#   Immunotherapy_Relevance = c("No", "No", "No", "No", "No", "Indirect", 
#                               "Direct", "No", "No", "No"),
#   Details = c("Involved in cell cycle regulation, no direct immunotherapy relevance",
#               "Transcription factor involved in DNA replication, no direct immunotherapy relevance",
#               "Involved in metabolism, no direct immunotherapy relevance",
#               "Associated with renal diseases, no direct immunotherapy relevance",
#               "Involved in vascular regulation, no direct immunotherapy relevance",
#               "Nuclear receptor involved in inflammation, potential relevance in modulating immune responses",
#               "Targeted in immunotherapy to block adenosine-mediated immunosuppression",
#               "Involved in nucleotide metabolism, no direct immunotherapy relevance",
#               "Involved in mitosis, no direct immunotherapy relevance",
#               "Transcription factor involved in development, no direct immunotherapy relevance"),
#   stringsAsFactors = FALSE
# )
# 
# # Add the sixth batch to the dataframe
# add_batch_results(batch_6)
# 
# 
# # Batch 7 results
# batch_7 <- data.frame(
#   Gene = c("PDIA3", "PDLIM7", "PIGR", "POSTN", "PRDM16-DT", "PRUNE2", 
#            "PSMB8", "PTPRD", "PXDN", "RGS3"),
#   Immunotherapy_Relevance = c("Indirect", "No", "No", "Indirect", "No", "No", 
#                               "Direct", "Indirect", "No", "No"),
#   Details = c("Involved in protein folding, may affect immune evasion, but not a direct immunotherapy target",
#               "Involved in cytoskeletal organization, no direct immunotherapy relevance",
#               "Involved in mucosal immunity, no direct cancer immunotherapy relevance",
#               "Involved in ECM remodeling, potential relevance in tumor microenvironment modulation",
#               "Long non-coding RNA, no direct immunotherapy relevance",
#               "Involved in tumor progression, no direct immunotherapy relevance",
#               "Plays a role in antigen presentation, relevant in enhancing immune recognition of tumors",
#               "Tumor suppressor, potential indirect relevance in immune modulation",
#               "Involved in collagen crosslinking, no direct immunotherapy relevance",
#               "Involved in GPCR signaling, no direct immunotherapy relevance"),
#   stringsAsFactors = FALSE
# )
# 
# # Add the seventh batch to the dataframe
# add_batch_results(batch_7)
# 
# # Batch 8 results
# batch_8 <- data.frame(
#   Gene = c("RHOB", "RRM2", "SAA2", "SCUBE2", "SIRPG", "SLC16A1", 
#            "SLC27A1", "SMARCA1", "SNHG3", "TANK"),
#   Immunotherapy_Relevance = c("Indirect", "No", "Indirect", "Indirect", "Direct", 
#                               "Indirect", "Indirect", "Indirect", "Indirect", "Direct"),
#   Details = c("Involved in apoptosis and cytoskeletal regulation, may affect immune-related signaling",
#               "Involved in DNA synthesis, no direct immunotherapy relevance",
#               "Involved in inflammation, potential relevance in immune modulation",
#               "Modulates Notch signaling, potential relevance in tumor progression",
#               "Modulates immune cell signaling, potential relevance in cancer immunotherapy",
#               "Involved in metabolic reprogramming, potential relevance in the tumor microenvironment",
#               "Involved in fatty acid metabolism, potential relevance in tumor microenvironment",
#               "Chromatin remodeling factor, may affect cancer progression but not directly targeted in immunotherapy",
#               "Long non-coding RNA involved in tumor progression, not directly targeted in immunotherapy",
#               "Involved in NF-κB signaling, regulates immune responses, potential relevance in immunotherapy"),
#   stringsAsFactors = FALSE
# )
# 
# # Add the eighth batch to the dataframe
# add_batch_results(batch_8)
# 
# # Batch 9 results
# batch_9 <- data.frame(
#   Gene = c("TIGIT", "TIMP4", "TMEM98", "TNFRSF4", "TRAF3IP3", "TSPAN2", 
#            "TUBA1C", "UBD", "VCAN", "ZAP70"),
#   Immunotherapy_Relevance = c("Direct", "Indirect", "No", "Direct", "Indirect", 
#                               "No", "No", "Indirect", "Indirect", "Direct"),
#   Details = c("Immune checkpoint receptor targeted in combination with PD-1/PD-L1 inhibitors",
#               "Involved in tissue remodeling, may affect immune cell infiltration",
#               "Involved in cell differentiation, no direct immunotherapy relevance",
#               "Co-stimulatory molecule targeted to enhance T-cell responses in immunotherapy",
#               "Modulates NF-κB signaling, may influence immune modulation",
#               "Involved in cell adhesion, no direct immunotherapy relevance",
#               "Involved in cell division, no direct immunotherapy relevance",
#               "Regulates protein degradation, potential relevance in immune responses",
#               "Involved in the tumor microenvironment, potential relevance in immune cell infiltration",
#               "Critical for T-cell receptor signaling and T-cell activation, relevant in cancer immunotherapy"),
#   stringsAsFactors = FALSE
# )
# 
# # Add the ninth batch to the dataframe
# add_batch_results(batch_9)
# 
# # Batch 10 results
# batch_10 <- data.frame(
#   Gene = c("ZBTB20", "ZFAND6"),
#   Immunotherapy_Relevance = c("No", "No"),
#   Details = c("Transcription factor involved in gene regulation, no direct immunotherapy relevance",
#               "Involved in protein degradation, no direct immunotherapy relevance"),
#   stringsAsFactors = FALSE
# )
# 
# # Add the tenth batch to the dataframe
# add_batch_results(batch_10)
# 
# # Adding the missing gene KNL1 to the dataframe
# missing_gene_data <- data.frame(
#   Gene = "KNL1",
#   Immunotherapy_Relevance = "No",
#   Details = "Involved in chromosome segregation during mitosis, no direct immunotherapy relevance",
#   stringsAsFactors = FALSE
# )
# 
# # Add the missing gene to the dataframe
# add_batch_results(missing_gene_data)
# 
# # Corrected snippet to select rows with "Direct" in Immunotherapy_Relevance
# direct_evidence <- immunotherapy_genes_df[immunotherapy_genes_df$Immunotherapy_Relevance == "Direct", ]
# 
# rio::export(direct_evidence,"direct_evidence.tsv")
# 
# # # Remove the "Overview" column from the dataframe
# # direct_evidence <- direct_evidence[, !(names(direct_evidence) %in% c("Overview"))]
# 
# # Correcting the "Overview" column to match the order in the original dataframe
# 
# direct_evidence$Overview <- c(
#   "CD244 (SLAMF4) has emerged as a target in immunotherapy, regulating NK and CD8+ T cells in the tumor microenvironment. Targeting CD244 can reverse immune exhaustion and improve immune-mediated tumor destruction.",
#   "CD8+ T cells are key to cancer immunotherapy, especially with immune checkpoint inhibitors (e.g., PD-1/PD-L1). CD8A is essential for cytotoxic T-cell responses.",
#   "CXCL10 enhances immune cell recruitment into the tumor microenvironment and is associated with improved responses to immunotherapies, boosting immune infiltration.",
#   "GZMB (Granzyme B) is an effector molecule in cytotoxic T and NK cells, inducing apoptosis in target cells, critical for immune-mediated tumor destruction.",
#   "KLRK1 (NKG2D) is a receptor on NK and T cells that recognizes stress-induced ligands on tumor cells. It is a promising target in immunotherapy strategies for enhancing immune surveillance.",
#   "NT5E (CD73) is targeted in immunotherapy to inhibit adenosine-mediated immunosuppression in the tumor microenvironment, enhancing antitumor immune responses.",
#   "PSMB8 (LMP7) plays a crucial role in antigen processing and presentation by MHC class I molecules, enhancing immune recognition of tumors in immunotherapy.",
#   "SIRPG modulates immune cell signaling, particularly in T and myeloid cells, with potential relevance in enhancing immune responses in cancer immunotherapy.",
#   "TANK is involved in regulating immune responses, including NF-κB signaling, and may have potential in cancer immunotherapy.",
#   "TIGIT is a major immune checkpoint receptor inhibiting T and NK cell activity. Blocking TIGIT, often in combination with PD-1/PD-L1 inhibitors, enhances antitumor immunity.",
#   "TNFRSF4 (OX40) is a co-stimulatory molecule that enhances T-cell survival and proliferation. OX40 agonists are being used in combination with checkpoint inhibitors to improve immune responses.
#   OX40 (TNFRSF4) is critical in immunotherapy, with OX40 agonists enhancing T-cell activity and survival in cancer treatments.",
#   "ZAP70 is involved in T-cell receptor signaling and is critical for T-cell activation and tumor recognition, playing a key role in boosting immune responses in cancer immunotherapy."
# )
# 
# # Correcting the number of references to match the 12 genes
# direct_evidence$Reference <- c(
#   'https://www.frontiersin.org/articles/10.3389/fimmu.2021.648182/full',  # CD244
#   'https://www.mdpi.com/2072-6694/14/19/4866',  # CD8A
#   'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7732061/',  # CXCL10
#   'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6161514/',  # GZMB
#   'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7197380/',  # KLRK1 (NKG2D)
#   'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7081619/',  # NT5E (CD73)
#   'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6314764/',  # PSMB8 (LMP7)
#   'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7834214/',  # SIRPG
#   'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7107290/',  # TANK: NF-κB signaling in cancer immunity
#   'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7379943/',  # TIGIT
#   'https://www.mdpi.com/2072-6694/14/19/4866', # TNFRSF4 (OX40)
#   'https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5952480/'  # ZAP70
#   )
# 
# # Order the dataframe "immunotherapy_genes_df" alphabetically by the "Gene" column
# immunotherapy_genes_df <- immunotherapy_genes_df[order(immunotherapy_genes_df$Gene), ]

# Remove leading and trailing spaces globally across all columns
# immunotherapy_genes_df <- immunotherapy_genes_df %>%
#   mutate_all(trimws)
 
# # Check if the values in the "Gene" column of "immunotherapy_genes_df" and "Gene_symbol" column of "Gene_Symbols_top_signatures" are identical
# identical(immunotherapy_genes_df$Gene, Gene_Symbols_top_signatures$Gene_Symbol)
# 
# 
# # Save the updated dataframe with the references
# write.csv(direct_evidence, "direct_evidence_with_correct_references.csv", row.names = FALSE)
...
###########
###########
#Extracting information via ChatGPT about possible immunotherapy value for the gene symbols from the "worst" gene signatures for being associated with pro-tumoral nd cold profiles

# # Load necessary libraries
# library(dplyr)
# 
# # Load your dataframe containing the 49 gene symbols
# gene_symbols <- read.table("Gene_Symbols_worst_signatures.tsv", header = TRUE, sep = "\t")
# 
# # Create a dataframe to store manually extracted information for each gene
# # The columns will include gene symbol, immunotherapy relevance, and notes on targeting
# all_immunotherapy_info <- data.frame(
#   Gene_Symbol = character(),
#   Immunotherapy_Relevance = character(),
#   Targeting_Notes = character(),
#   stringsAsFactors = FALSE
# )
# 
# # Define a function to process each batch of genes and accumulate manually extracted info
# process_manual_batch <- function(gene_batch, batch_info) {
#   # Append manually extracted information to the cumulative dataframe
#   batch_df <- data.frame(
#     Gene_Symbol = gene_batch,
#     Immunotherapy_Relevance = batch_info$Immunotherapy_Relevance,
#     Targeting_Notes = batch_info$Targeting_Notes,
#     stringsAsFactors = FALSE
#   )
#   
#   # Append to the main dataframe
#   return(rbind(all_immunotherapy_info, batch_df))
# }
# 
# # Batch 1 (Manually Extracted Information from GPT)
# batch_1_info <- data.frame(
#   Immunotherapy_Relevance = c(
#     "Potential role in tumor microenvironment, indirect relevance",
#     "Tumor microenvironment modulation, indirect relevance",
#     "Emerging evidence in angiogenesis, indirect relevance",
#     "Role in modulating the tumor microenvironment",
#     "Effector in apoptosis, complementary to immunotherapy",
#     "Cell cycle regulation, under investigation for combination therapies",
#     "Reprogramming tumor-associated macrophages, target in immunotherapy",
#     "Modulating immune response in tumor microenvironment, potential target",
#     "ESCRT complex role, potential relevance in immune surveillance",
#     "Tight junction protein, potential relevance in enhancing immunotherapies"
#   ),
#   Targeting_Notes = c(
#     "ADAM12 may be relevant for combination with immune checkpoint inhibitors.",
#     "ADAMTS12 may be targeted indirectly through the tumor stroma.",
#     "AMIGO2 targeting under study for vascularization and combination approaches.",
#     "ANO1 targeting could disrupt tumor microenvironment interactions.",
#     "CASP7 involved in apoptosis, may enhance cell death in cancer therapies.",
#     "CCND1 overexpression may sensitize tumors to immune checkpoint blockade.",
#     "CD36 targeting under study for macrophage reprogramming.",
#     "CD97 could modulate immune cell interactions, under study.",
#     "CHMP4C in ESCRT pathway, potential immune relevance.",
#     "CLDN1 targeting tight junctions to enhance immunotherapy delivery."
#   )
# )
# 
# # Manually extract information for batch 1
# gene_batch_1 <- gene_symbols$Gene_Symbol[1:10]
# all_immunotherapy_info <- process_manual_batch(gene_batch_1, batch_1_info)
# 
# # Save progress after batch 1
# saveRDS(all_immunotherapy_info, file = "immunotherapy_info_batch_1.rds")
# write.table(all_immunotherapy_info, file = "all_immunotherapy_info.tsv", sep = "\t", row.names = FALSE)
# 
# # Print status message
# print("Batch 1 processed and saved.")
# 
# # Continue with batch 2 manually (for future batches)
# 
# # Batch 2 (Manually Extracted Information)
# batch_2_info <- data.frame(
#   Immunotherapy_Relevance = c(
#     "Antigen presentation and immune modulation, potential target in immunotherapy",
#     "Extracellular matrix involvement, indirect relevance in combination with immunotherapy",
#     "Extracellular matrix involvement, potential for enhancing stroma-targeted immunotherapy",
#     "Immune cell recruitment in tumor microenvironment, direct relevance in immunotherapy",
#     "Regulates cytoskeleton dynamics, indirect relevance in enhancing cancer therapies",
#     "Notch signaling ligand, target for antibody-drug conjugates in cancer immunotherapy",
#     "Involved in extracellular matrix integrity, enhancing immunotherapy efficacy",
#     "Well-established target in cancer, potential for combination with immunotherapies",
#     "Modulates immune cell interactions, target in immunotherapy",
#     "Involved in immune modulation, being studied for immunotherapy applications"
#   ),
#   Targeting_Notes = c(
#     "CLEC4A can modulate antigen presentation and may enhance immunotherapy responses.",
#     "COL11A1 targeting may disrupt tumor invasion, complementing immune checkpoint therapies.",
#     "COL12A1 could be targeted in combination with stroma-disrupting therapies.",
#     "CXCL14 may be used to attract immune cells to the tumor, enhancing immunotherapy efficacy.",
#     "CYFIP2's role in cell motility could enhance therapies targeting cancer metastasis.",
#     "DLL3 is a promising target for antibody-drug conjugates in small cell lung cancer.",
#     "ECM1 modulation could enhance the impact of immunotherapy in solid tumors.",
#     "EGFR inhibitors may be combined with immunotherapies for enhanced efficacy.",
#     "EPHA2 is being studied for its role in tumor-immune interactions and as a target.",
#     "EPHB2 may be targeted to modulate immune cell activity in tumors."
#   )
# )
# 
# # Manually extract information for batch 2
# gene_batch_2 <- gene_symbols$Gene_Symbol[11:20]
# all_immunotherapy_info <- process_manual_batch(gene_batch_2, batch_2_info)
# 
# # Save progress after batch 2
# saveRDS(all_immunotherapy_info, file = "immunotherapy_info_batch_2.rds")
# write.table(all_immunotherapy_info, file = "all_immunotherapy_info.tsv", sep = "\t", row.names = FALSE)
# 
# # Print status message
# print("Batch 2 processed and saved.")
# 
# # Batch 3 (Manually Extracted Information)
# batch_3_info <- data.frame(
#   Immunotherapy_Relevance = c(
#     "Involved in DNA damage response, may sensitize tumors to immunotherapy",
#     "Structural component of extracellular matrix, indirect relevance in tumor stroma targeting",
#     "Targeted in combination therapies with immunotherapy for immune evasion",
#     "Target in angiogenesis inhibition, combined with immune checkpoint inhibitors",
#     "Modulates transcriptional programs, potential to enhance immunotherapy",
#     "Involved in immune modulation, potential indirect target in combination therapies",
#     "Alters tumor immune microenvironment, indirect relevance in immunotherapy",
#     "Limited direct evidence, potential role in immune evasion through glycosylation",
#     "Target in hypoxic microenvironment, relevance in immune resistance modulation",
#     "Facilitates immune cell infiltration, direct target in immunotherapy"
#   ),
#   Targeting_Notes = c(
#     "EYA4 may be targeted to enhance DNA repair inhibition in combination with immune checkpoint blockade.",
#     "FBN2 may be targeted in combination with therapies disrupting the tumor stroma.",
#     "FGFR4 inhibitors are being tested in combination with immunotherapies.",
#     "FLT1 inhibitors are combined with immune checkpoint inhibitors to enhance immune cell infiltration.",
#     "FOSL1 may be targeted to suppress transcription factors driving tumor progression.",
#     "FSTL1 is involved in inflammatory responses, complementing immune-modulating therapies.",
#     "GDF15 is being explored to reprogram immune responses in the tumor microenvironment.",
#     "GLT8D1’s involvement in glycosylation could be linked to immune evasion, though evidence is limited.",
#     "HIF3A targeting is being studied for modulating hypoxic tumor resistance to immune cells.",
#     "ICAM1 is a key target in enhancing immune cell recruitment to tumors."
#   )
# )
# 
# # Manually extract information for batch 3
# gene_batch_3 <- gene_symbols$Gene_Symbol[21:30]
# all_immunotherapy_info <- process_manual_batch(gene_batch_3, batch_3_info)
# 
# # Save progress after batch 3
# saveRDS(all_immunotherapy_info, file = "immunotherapy_info_batch_3.rds")
# write.table(all_immunotherapy_info, file = "all_immunotherapy_info.tsv", sep = "\t", row.names = FALSE)
# 
# # Print status message
# print("Batch 3 processed and saved.")
# 
# # Batch 4 (Manually Extracted Information)
# batch_4_info <- data.frame(
#   Immunotherapy_Relevance = c(
#     "Involved in immune detection, potential target in immunotherapy for enhancing tumor immune recognition",
#     "Pro-inflammatory cytokine, targeting IL-6 can enhance immune checkpoint inhibitor efficacy",
#     "Targeted in therapies disrupting tumor-stroma interactions, enhancing immunotherapy",
#     "Modulates transcriptional programs in cancer, potential to enhance immunotherapy responses",
#     "Transcription factor involved in cancer progression, potential target for altering tumor dynamics in combination with immunotherapy",
#     "Disrupts tumor-stroma interactions, potential to enhance immune infiltration in tumors",
#     "Involved in cancer cell invasion and autophagy, potential target to enhance immune-mediated tumor clearance",
#     "Modulates immune microenvironment, potential relevance in combination with immunotherapies",
#     "Immunomodulatory protein, direct target in immunotherapy to enhance anti-tumor immunity",
#     "Involved in extracellular matrix remodeling, potential relevance in combination immunotherapy strategies"
#   ),
#   Targeting_Notes = c(
#     "IFI16 enhances immune recognition of tumors through DNA sensing and signaling pathways.",
#     "IL6 blockade is being combined with immune checkpoint inhibitors to prevent immune evasion.",
#     "ITGB1 is under investigation for therapies targeting tumor-stroma interactions.",
#     "JUN modulation may improve the transcriptional environment for immune-based therapies.",
#     "KLF5 targeting could alter cancer growth and support immune-based treatments.",
#     "LAMA4 disruption could enhance immune cell infiltration by breaking down the tumor stroma.",
#     "LAMP1 is a potential target for enhancing autophagy and immune clearance of cancer cells.",
#     "LEPR could be targeted to modulate the immune microenvironment in certain cancer types.",
#     "LGALS9 is being targeted to block immunosuppressive interactions in tumor environments.",
#     "LOX modulation could complement immunotherapies by affecting the extracellular matrix."
#   )
# )
# 
# # Manually extract information for batch 4
# gene_batch_4 <- gene_symbols$Gene_Symbol[31:40]
# all_immunotherapy_info <- process_manual_batch(gene_batch_4, batch_4_info)
# 
# # Save progress after batch 4
# saveRDS(all_immunotherapy_info, file = "immunotherapy_info_batch_4.rds")
# write.table(all_immunotherapy_info, file = "all_immunotherapy_info.tsv", sep = "\t", row.names = FALSE)
# 
# # Print status message
# print("Batch 4 processed and saved.")
# 
# # Batch 5 (Manually Extracted Information)
# batch_5_info <- data.frame(
#   Immunotherapy_Relevance = c(
#     "Involved in extracellular matrix remodeling, potential relevance in enhancing immune infiltration",
#     "Cancer/testis antigen, direct target in immunotherapies, particularly cancer vaccines",
#     "Target in tumor invasion and metastasis, complementing immunotherapies",
#     "Supports basement membrane integrity, potential relevance in stroma-targeting therapies",
#     "Target in cancer therapies, especially in combination with immunotherapies",
#     "Modulates tumor microenvironment, enhancing immune responses and supporting immunotherapies",
#     "Involved in cell-cell adhesion, potential indirect relevance in immunotherapy",
#     "Interacts with immune checkpoints, direct target in immunotherapy",
#     "Involved in immune evasion, potential to restore immune surveillance in combination therapies"
#   ),
#   Targeting_Notes = c(
#     "LOXL1 targeting may complement therapies aiming to disrupt the tumor stroma.",
#     "MAGEA3 is a direct target in T-cell therapies and cancer vaccines.",
#     "MMP11 inhibition could enhance immune cell infiltration into tumors.",
#     "NID2 targeting may enhance therapies that disrupt tumor-stroma interactions.",
#     "PDGFRB inhibitors are used in combination with immune checkpoint blockade.",
#     "POSTN is a target for disrupting the tumor microenvironment to support immunotherapy.",
#     "PPFIA1 may influence immune cell interactions, though its role in immunotherapy is still being explored.",
#     "PVR is a target for blocking immune checkpoints and enhancing T-cell activity.",
#     "RNF43 targeting is being studied for its potential to restore immune function in tumors."
#   )
# )
# 
# # Manually extract information for batch 5
# gene_batch_5 <- gene_symbols$Gene_Symbol[41:49]
# all_immunotherapy_info <- process_manual_batch(gene_batch_5, batch_5_info)

# # Save progress after batch 5
# saveRDS(all_immunotherapy_info, file = "immunotherapy_info_batch_5.rds")
# write.table(all_immunotherapy_info, file = "all_immunotherapy_info.tsv", sep = "\t", row.names = FALSE)
# 
# # Print status message
# print("Batch 5 processed and saved.")
# # Save the workspace to the current working directory (as .RData file)
...
# df102 <-signatures_for_figures
# 
# # Select specific columns from df102 and create df03 with the corrected order
# df103 <- df102[, c("Ranking", "Nomenclature", "Signature", "Genotype", "RCD_types")]
# # Rename the columns in df03 and order it by "Rank" in descending order
# df103 <- df103 %>%
#   rename(
#     Rank = Ranking,               # "Ranking" to "Rank"
#     `RCD forms` = RCD_types,       # "RCD_types" to "RCD forms"
#     `Genomic feature` = Genotype   # "Genotype" to "Genomic feature"
#   ) %>%
#   arrange(desc(Rank))  # Sort by "Rank" in descending order
# 
# rio::export(df103, "signatures_for_figures_7_final.tsv")
...
write.xlsx(df113, "Multi_RCD_specific_per_genotype_class_selected_signatures.xlsx")
...
write.xlsx(df113_ordered, "Multi_RCD_specific_per_genotype_class_selected_signatures_with_Priorities.xlsx")

# Save the top 20 rows to a separate Excel file
write.xlsx(df113_top20, "Top20_Multi_RCD_specific_per_genotype_class_selected_signatures_Genotype_Prioritized_Rows.xlsx")
...
# Save the top 20 rows to a separate Excel file
write.xlsx(df1011_select_updated_top20, "Top20_Multi_RCD_specific_per_genotype_class_selected_signatures_Genotype_Prioritized_Rows.xlsx")
...
write.xlsx(final_renamed_ordered_df1011, "Renamed_Ordered_Top_20_Ranked_Signatures_df1011.xlsx")
...
write.xlsx(
  selected_columns_df1047_reordered, 
  "Table 2. Top-ranked RCD type-specific signatures with comprehensive multi-omic representation_reordered.xlsx", 
  rowNames = FALSE
)
...
write.xlsx(
  selected_columns_combined_genotype_rows, 
  "Table 3. Seven top-ranked signatures by multi-omic feature.xlsx", 
  rowNames = FALSE
)
...
####
####
# sub-setting df1157 by genotype values to created genotype-specifis dataset1
library(dplyr)
library(openxlsx)
library(readr)

# Step 1: Get unique "Genotype" values
unique_genotypes_df1157 <- unique(df1157$Genotype)

# Step 2: Loop through each unique genotype to create and save separate datasets
for (genotype in unique_genotypes_df1157) {
  # Create a filtered dataframe for the current genotype
  genotype_df <- df1157 %>% filter(Genotype == genotype)
  
  # Assign dataframe to a dynamically created object in the environment
  assign(paste0("df1157_", genotype), genotype_df, envir = .GlobalEnv)
  
  # Define file names for both .xlsx and .tsv formats
  file_name_xlsx <- paste0("Dataset_S1_", genotype, ".xlsx")
  file_name_tsv  <- paste0("Dataset_S1_", genotype, ".tsv")
  
  # Save dataframe to an .xlsx file
  write.xlsx(genotype_df, file_name_xlsx, overwrite = TRUE)
  
  # Save dataframe to a .tsv file
  write_tsv(genotype_df, file_name_tsv)
...
# Print final message
print("All genotype-specific files and objects created successfully!")

######
######
######
###### Global analysis of dataframes by genotype ######
###### 
###### 
library(dplyr)
...
#####
#####
##### Creating most clinical meaningful signatures for validaion by precog analysis
#####
#####
##### 
...
##### 
##### 
##### Distribution of genityoe vakues wr CTAB lookup for theCTAB withmaimum signaturues per genotyoe
##### Top Cancer Type for Each Genotype
##### 
##### 
# Load necessary library
library(dplyr)

# Group by CTAB and Genotype, then count occurrences
...
df_max_counts <- df_distribution %>%
  ...
  ungroup() %>%
  select(CTAB, Genotype, Absolute_Count) %>%
...
####
####
# Reanalysing sub-setting df1157 by `Omic feature` values to created omic feature-specifis dataset_S1_
...
# Save workbook
saveWorkbook(wb, "Transcript_S1_b_Optimized_Supplementary_Data_with_Selective_Hyperlinks_v02.xlsx", overwrite = TRUE)
...
# Relocate "Count_source" immediately after "Ranking"
df165 <- df165 %>%
  relocate(Elements, .after = Ranking)
...
# Relocate "Elements" immediately after "Ranking"
df165 <- df165 %>%
  relocate(Elements, .after = Ranking)
...
library(rio)  # For exporting .tsv files
...
# Target_genes <- data.frame(`RCD form` = c("Apoptosis/Cellular senescence", "Necrosis", "Apoptosis", "Cellular senescence/Necrosis"))
