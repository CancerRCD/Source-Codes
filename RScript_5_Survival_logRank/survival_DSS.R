...
run_analysis <- function(gene_ID, gene_var, tumor) {
  ...
  surv_diff <- tryCatch({
    survdiff(Surv(DSS.time, DSS) ~ group, data = data_surv)
  }, error = function(e) {
  ...
  compare_groups <- function(data, group1, group2) {
    ...
    surv_diff_pair <- tryCatch({
      survdiff(Surv(DSS.time, DSS) ~ group, data = subset_data)
    }, error = function(e) {
...
checkpoint_file <- "progress_checkpoint_DSS.rds"
...
write.table(results_df, "survival_analysis_all_signature_DSS.tsv", sep="\t", row.names=FALSE, quote=FALSE)
