...
run_analysis <- function(gene_ID, gene_var, tumor) {
  ...
  surv_diff <- tryCatch({
    survdiff(Surv(OS.time, OS) ~ group, data = data_surv)
  }, error = function(e) {
  ...
  compare_groups <- function(data, group1, group2) {
    ...
    surv_diff_pair <- tryCatch({
      survdiff(Surv(OS.time, OS) ~ group, data = subset_data)
    }, error = function(e) {
...
checkpoint_file <- "progress_checkpoint_OS.rds"
...
write.table(results_df, "survival_analysis_all_signature_OS.tsv", sep="\t", row.names=FALSE, quote=FALSE)
