...
## Protein vs TMB
## Note: protein database is restricted to an approximately 300 protein panel known to be impacted in cancer, so expect lost of NA data

## Function 1: Define a wrapper function
vis_gene_wrapper <- function(sig) {
  
  p = vis_gene_tmb_cor(Gene = sig, data_type = "protein", cor_method = "spearman")
  p[["data"]] <- p[["data"]] %>%
...
# ## Function 4: Run gene TMB
# get_tmb_data = function(genes){
#   
