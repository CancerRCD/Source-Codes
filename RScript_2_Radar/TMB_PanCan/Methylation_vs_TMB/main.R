...
getwd()
setwd("C:/Users/Emamnuell/Desktop/panoptosis/genes/genes_24_optosis/codigos/2°_Radar/TMB_PanCan/Methylation_vs_TMB")
...
tmb_gene_data = read_rds("int/Target_genes.rds")

####
# Filter out genes with null data
empty_gene_names <- names(tmb_gene_data)[sapply(tmb_gene_data, function(x) all(is.na(x)))]
...
####
empty_gene_names = names(tmb_gene_data[which(sapply(tmb_gene_data, is.null))])
if (length(empty_gene_names) > 0){
  tmb_gene_data = tmb_gene_data[-which(sapply(tmb_gene_data, is.null))]
}

gene_sig_list = lapply(tmb_gene_data, pivot_wider_manager)
...
write.table(empty_gene_names,
            file = "int/null_genes_pancan_tmb_radar.tsv",
            sep = "\t",
            quote = FALSE,
            col.names = FALSE,
            row.names = FALSE)
...
write.table(gene_sig_table,
            file = "results/sig_genes_corr_pancan_tmb.tsv",
            sep = "\t",
            quote = FALSE,
            row.names = FALSE)  # No longer using row.names, as "Genes" is now a regular column

##################################### SET NETWORK ----
network_table <- fread('results/sig_genes_corr_pancan_tmb.tsv')
...
# Loop through each gene and add p.adj values from tmb_gene_data
for (gene in unique(network_table$genes)) {
  if (gene %in% names(tmb_gene_data)) {
    # Extract all p.adj values for the current gene
    Correlation_p.adj <- tmb_gene_data[[gene]][["padj"]]
