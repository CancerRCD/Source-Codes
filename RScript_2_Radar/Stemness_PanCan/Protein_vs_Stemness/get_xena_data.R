...
## Protein vs Stemness
## Note: protein database is restricted to an approximately 300 protein panel known to be impacted in cancer, so exdpect lost of NA data
...
rds_xena_stemness_name_output <- "int/Target_genes.rds"
...
get_gene_data <- function(gene) {
  data <- try(get_stemness_data(gene)$data, silent = TRUE)
...
if (length(current_genes) <= N_FOR_SPLIT) {
  # Data with <= 500 genes
  current_stemness_genes <- get_stemness_data(current_genes)
  gene_data <- lapply(current_stemness_genes, function(x) x$data)
  
  saveRDS(gene_data, rds_xena_stemness_name_output)
  ...
  for (sub in subsets) {
    ...
    current_stemness_genes <- get_stemness_data(current_subset)
    gene_data <- lapply(current_stemness_genes, function(x) x$data)
    
    full_list <- c(full_list, gene_data)
    
    saveRDS(full_list, rds_xena_stemness_name_output)
  }
