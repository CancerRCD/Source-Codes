...
getwd()
setwd("C:/Users/Emamnuell/Desktop/panoptosis/genes/genes_24_optosis/codigos/2°_Radar/Stemness_PanCan/Methylation_vs_Stemness")
...
stemness_gene_data = read_rds("int/Target_genes.rds")

####
# Filter out genes with null data
empty_gene_names <- names(stemness_gene_data)[sapply(stemness_gene_data, function(x) all(is.na(x)))]
...
####
empty_gene_names = names(stemness_gene_data[which(sapply(stemness_gene_data, is.null))])
if (length(empty_gene_names) > 0){
  stemness_gene_data = stemness_gene_data[-which(sapply(stemness_gene_data, is.null))]
}

gene_sig_list = lapply(stemness_gene_data, pivot_wider_manager)
...
write.table(empty_gene_names,
            file = "int/null_genes_pancan_stemness_radar.tsv",
            sep = "\t",
            quote = FALSE,
            col.names = FALSE,
            row.names = FALSE)
...
write.table(gene_sig_table,
            file = "results/sig_genes_corr_pancan_stemness.tsv",
            sep = "\t",
            quote = FALSE,
            row.names = FALSE)  # No longer using row.names, as "Genes" is now a regular column

##################################### SET NETWORK ----
network_table <- fread('results/sig_genes_corr_pancan_stemness.tsv')
...
# Loop through each gene and add p.adj values from stemness_gene_data
for (gene in unique(network_table$genes)) {
  if (gene %in% names(stemness_gene_data)) {
    # Extract all p.adj values for the current gene
    Correlation_p.adj <- stemness_gene_data[[gene]][["padj"]]
